...
tuple_names = ['ctx', 'simple_prompt', 'prompt', 'negative_prompt', 'data_model', 'steps', 'width', 'height',
               ...
               'highres_fix', 'full_quality_vae','clip_skip', 'extra_net', 'spoiler', 'epoch_time']
...
class DrawView(View):
    def __init__(self, input_tuple):
        ...
        if isinstance(self.input_tuple, tuple): # only check batch if we are actually a real view
            ...
            if batch_count > 1:
                download_menu = DownloadMenu(input_tuple[21], input_tuple[10], batch_count, input_tuple)
                download_menu.callback = download_menu.callback
                self.add_item(download_menu)
                upscale_menu = UpscaleMenu(input_tuple[21], input_tuple[10], batch_count, input_tuple)
